    ...
    <section className="ticket">
      ...
      {ticket.picks.map((g,i) => (
        <div className="pick" key={i}>
          ...
          <div>
            <b>{g.home} x {g.away}</b>
            <small>{g.pickLine || defaultLine} • Score {scoreFor(g,i)}</small>
          </div>
...
export default function PradoIA(){
  ...
  async function makeScannerTicket(){
    ...
    if(!t){
      const oddTotal = picks.reduce((a,g) => a * g.odd, 1);
      const avgProb = Math.round(picks.reduce((a,g) => a + g.prob, 0) / picks.length);
      t = { picks, oddTotal, avgProb, line:curLine };
    } else {
      t = { ...t, picks:picks.map((g,i) => ({ ...g, pickLine: curLine })) };
    }
  ...
  async function makeRiskTicket(openTab = true){
    ...
    const picks = sortGamesForRisk(games, risk)
      .slice(0, cfg.qty)
      .map((g,i) => ({ ...g, pickLine: marketForPick(risk, i) }));

    let t = await getTicket({
      games:picks,
      qty:picks.length,
      risk,
      line:'Mercados variados'
    });

    if(!t){
      const oddTotal = picks.reduce((a,g) => a * g.odd, 1);
      const avgProb = Math.round(picks.reduce((a,g) => a + g.prob, 0) / picks.length);
      t = { picks, oddTotal, avgProb, line:'Mercados variados' };
    } else {
      t = { ...t, picks };
    }
